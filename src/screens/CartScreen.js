...
function CartScreen() {
  ...
  const qty = new URLSearchParams(location.search).get("qty") || 1; // Extract qty from query string
 ...
 const { userInfo } = userLogin;
  ...
  const checkoutHandler = () => {
    if (!userInfo) {
      // If the user is not logged in, redirect to login with redirect to shipping
      navigate("/login");
    } else {
